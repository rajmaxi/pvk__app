...
import HomePlaceHolder from '../../components/placeholder'
const Home: HomeComponentType = (HomeComponentProps) => {
  ...
  // const _cartDatas = async () => {
  //   // console.log('load12');
  //   if (loadNumber == 1 && singleLoad) {
  //     setSingleLoad(false);
  //     // console.log('load1');
  //     // Alert.alert('log check')
  //     var addedItems = await AsyncStorage.getItem('cartData');
  //     console.log("null", addedItems);
  //     if (addedItems != null) {
  //       console.log("null not", addedItems);
  //       var cartStrng = JSON.parse(addedItems);
  //       cartStrng.forEach(element => {
  //         dispatch({ type: 'ADDTOCART', payload: element })
  //       });
  ...
  const ShowHomeContent = () => {
    return (
      <SafeAreaView style={{ flex: 1 }}>
        {/* <TouchableOpacity onPress={_clearLocal}>
          <Text>HI cartPage How r u</Text>
        </TouchableOpacity> */}
  ...
  const RenderHome = () => {
    return (
      <View>
        {
          (homeresponse.banners[0].length != 0) ? (<>
...
{RenderTitle('shopbybrand')} */}
  ...
  return (
    (getloading == false) ? <HomePlaceHolder /> : ShowHomeContent()
  )
